...
def period_bounds(period: str, anchor: date | None = None, today: date | None = None) -> tuple[date, date]:
    ...
    if period == "quarterly":
        ...
        end_month = month + 2
        return start, date(current.year, end_month, monthrange(current.year, end_month)[1])
    if period == "annual":
        return date(current.year, 1, 1), date(current.year, 12, 31)
    start = date(current.year, current.month, 1)
    return start, date(current.year, current.month, monthrange(current.year, current.month)[1])
...
def update_category(db: DbSession, user: User, category_id: int, payload: dict[str, Any]) -> dict:
    ...
    db.commit()
    return [item for item in list_categories(db, user) if item["id"] == category_id][0]
...
def create_budget(db: DbSession, user: User, payload: dict[str, Any]) -> dict:
    ...
    if relationship not in RELATIONSHIP_MODES:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid relationship mode")
    allocation = payload.get("allocation_strategy") or "spend_during_period"
    if allocation not in ALLOCATION_STRATEGIES:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid allocation strategy")
    ...
    db.commit()
    return list_budgets(db, user, True)[-1]
...
def update_budget(db: DbSession, user: User, budget_id: int, payload: dict[str, Any]) -> dict:
    ...
    db.execute(text("""
        ...
        WHERE user_id=:user_id AND id=:id
    """), {"user_id": user.id, "id": budget_id, "name": payload.get("name", row["name"]), "category_id": payload.get("category_id", row["category_id"]), "category_name": payload.get("category_name", row["category_name"]), "direction": payload.get("direction", row["direction"]), "period": period, "amount": amount, "allocation": allocation, "relationship": payload.get("relationship_mode", row["relationship_mode"]), "anchor": _as_date(payload.get("anchor_date")) or row["anchor_date"], "start_date": _as_date(payload.get("start_date")) or row["start_date"], "end_date": _as_date(payload.get("end_date")) or row["end_date"], "rollover": payload.get("rollover_enabled", row["rollover_enabled"]), "negative_rollover": payload.get("negative_rollover_enabled", row["negative_rollover_enabled"]), "notes": payload.get("notes", row["notes"]), "is_active": payload.get("is_active", row["is_active"]), "now": now})
    if effective_from:
        db.execute(text("INSERT INTO budget_versions (budget_id, user_id, amount_cents, period, allocation_strategy, effective_from, created_at) VALUES (:budget_id, :user_id, :amount, :period, :allocation, :effective_from, :now)"), {"budget_id": budget_id, "user_id": user.id, "amount": amount, "period": period, "allocation": allocation, "effective_from": effective_from, "now": now})
    db.commit()
    return [item for item in list_budgets(db, user, True) if item["id"] == budget_id][0]
...
def _actual_for_categories(db: DbSession, user: User, names: set[str], start: date, end: date, direction: str, account_ids: list[int] | None = None) -> tuple[int, int]:
    if not names:
        return 0, 0
    placeholders = {f"c{i}": name for i, name in enumerate(names)}
    account_sql = ""
    params: dict[str, Any] = {"user_id": user.id, "start": start, "end": end, **placeholders}
    if account_ids:
        account_placeholders = {f"a{i}": account_id for i, account_id in enumerate(account_ids)}
        account_sql = " AND account_id IN (" + ",".join(f":{k}" for k in account_placeholders) + ")"
        params.update(account_placeholders)
    tx_type = "income" if direction == "income" else "expense"
    row = db.execute(text(f"SELECT COUNT(*) AS count, COALESCE(SUM(amount_cents),0) AS total FROM transactions WHERE user_id=:user_id AND transaction_type=:type AND transaction_date BETWEEN :start AND :end AND category IN ({','.join(':'+k for k in placeholders)}) {account_sql}"), {**params, "type": tx_type}).mappings().first()
    return int(abs(row["total"] or 0)), int(row["count"] or 0)


def _scheduled_for_categories(db: DbSession, user: User, names: set[str], start: date, end: date, source_type: str | None = None) -> tuple[int, int, list[dict]]:
    forecast = generate_forecast(db, user, f"{(end - start).days + 1}d", "expected", start)
    rows = []
    for item in forecast.get("timeline", []):
        if item.get("direction") != "expense":
            continue
        if item.get("category") not in names:
            continue
        if source_type and item.get("source_type") != source_type:
            continue
        rows.append(item)
    return sum(abs(parse_money(item["amount"])) for item in rows), len(rows), rows
...
def analyse_budgets(db: DbSession, user: User, start: date | None = None, end: date | None = None, category_id: int | None = None, account_ids: list[int] | None = None, mode: str = "native") -> dict:
    today = _today()
    budgets = list_budgets(db, user)
    if category_id:
        budgets = [b for b in budgets if b.get("category_id") == category_id]
    rows = []
    total_budget = total_actual = total_committed = total_planned = total_forecast = 0
    for budget in budgets:
        native_start, native_end = period_bounds(budget["period"], _as_date(budget.get("anchor_date")), today)
        ...
        if budget.get("allocation_strategy") == "spread_weekly" and budget["period"] == "annual":
            applicable = _round_cents(Decimal(base_budget) / Decimal(52)) if mode == "native" and not start else applicable
        elif budget.get("allocation_strategy") == "spread_fortnightly" and budget["period"] == "annual":
            applicable = _round_cents(Decimal(base_budget) / Decimal(26)) if mode == "native" and not start else applicable
        elif budget.get("allocation_strategy") == "spread_monthly" and budget["period"] == "annual":
            applicable = _round_cents(Decimal(base_budget) / Decimal(12)) if mode == "native" and not start else applicable
        ...
        actual, actual_count = _actual_for_categories(db, user, names, period_start, period_end, budget["direction"], account_ids)
        committed, committed_count, _ = _scheduled_for_categories(db, user, names, period_start, period_end)
        planned, planned_count, _ = _scheduled_for_categories(db, user, names, period_start, period_end, "planned_spending")
        ...
        period_elapsed = _period_elapsed_percent(period_start, period_end, today)
        if actual > available:
            status_name = "over_budget"
        elif forecast_total > available:
            status_name = "projected_over_budget"
        elif utilisation >= 85:
            status_name = "approaching_limit"
        else:
            status_name = "on_track"
        item = {
            "id": budget["id"], "name": budget["name"], "category": budget.get("category_path") or budget.get("category_name"),
            "period": budget["period"], "direction": budget["direction"], "relationship_mode": budget["relationship_mode"],
            "allocation_strategy": budget["allocation_strategy"], "period_start": period_start.isoformat(), "period_end": period_end.isoformat(),
            "base_budget": cents_to_decimal(applicable), "rollover": cents_to_decimal(rollover), "available_budget": cents_to_decimal(available),
            "actual": cents_to_decimal(actual), "committed": cents_to_decimal(committed), "planned": cents_to_decimal(planned),
            "forecast": cents_to_decimal(forecast_total), "current_remaining": cents_to_decimal(current_remaining),
            "projected_remaining": cents_to_decimal(projected_remaining), "projected_variance": cents_to_decimal(variance),
            "utilisation_percent": utilisation, "period_elapsed_percent": period_elapsed, "status": status_name,
            "counts": {"actual": actual_count, "committed": committed_count, "planned": planned_count, "forecast": committed_count + planned_count},
            "explanation": f"Actual is based on {actual_count} transactions. Forecast combines actual, committed and planned items for {period_start.isoformat()} to {period_end.isoformat()}.",
        }
        rows.append(item)
    ...
    unbudgeted = find_unbudgeted_categories(db, user, start or today.replace(day=1), end or today)
    return {
        "mode": mode, "start": (start or today).isoformat(), "end": (end or today).isoformat(),
        "summary": {"base_budget": cents_to_decimal(total_budget), "actual": cents_to_decimal(total_actual), "committed": cents_to_decimal(total_committed), "planned": cents_to_decimal(total_planned), "forecast": cents_to_decimal(total_forecast), "projected_variance": cents_to_decimal(total_forecast - total_budget), "budgeted_surplus_deficit": cents_to_decimal(total_budget - total_forecast)},
        "budgets": rows, "unbudgeted_categories": unbudgeted,
    }
...
def save_view(db: DbSession, user: User, payload: dict[str, Any]) -> dict:
    ...
    if existing:
        db.execute(text("UPDATE saved_views SET settings_json=:settings, updated_at=:now WHERE id=:id AND user_id=:user_id"), {"id": existing, "user_id": user.id, "settings": body, "now": now})
    else:
        db.execute(text("INSERT INTO saved_views (user_id, screen, name, settings_json, is_default, created_at, updated_at) VALUES (:user_id, :screen, :name, :settings, :is_default, :now, :now)"), {"user_id": user.id, "screen": screen, "name": name, "settings": body, "is_default": bool(payload.get("is_default", False)), "now": now})
    db.commit()
