...
class AuthInitResult:
    ...
    def public(self) -> dict[str, object]:
        return {
            ...
            if self.state in {AuthLifecycleState.READY, AuthLifecycleState.READY_WITH_CONFIGURATION_WARNING}
            else "not_ready",
...
def _persist_result(db: DbSession, result: AuthInitResult) -> None:
    ...
    _set_app_config(
        ...
        result.message
        if result.state
        in {AuthLifecycleState.AUTH_CONFIGURATION_ERROR, AuthLifecycleState.READY_WITH_CONFIGURATION_WARNING}
        else "",
    )
...
def _apply_recovery(
    ...
    cleared = (
        db.execute(delete(LoginAttempt).where(LoginAttempt.username.in_({old_username, username}))).rowcount
        or 0
    )
    return int(revoked), int(cleared)


def _configuration_error(db: DbSession, action: str, message: str) -> AuthInitResult:
    result = AuthInitResult(AuthLifecycleState.AUTH_CONFIGURATION_ERROR, action, message)
...
def initialize_authentication(
    ...
    *,
    ...
    LOGGER.info(
        "Authentication initialisation started users=%s admins=%s recovery_mode=%s",
        len(users),
        len(admins),
        settings.admin_recovery_mode,
    )
    ...
    if not bootstrap_configured(settings):
        state = AuthLifecycleState.BOOTSTRAP_REQUIRED if not users else AuthLifecycleState.READY
        result = AuthInitResult(
            state=state,
            action="none",
            message=(
                "Administrator bootstrap configuration is not supplied."
                if not users
                else "Authentication ready using persisted administrator credentials."
            ),
        )
        with db.begin_nested():
            _persist_result(db, result)
        db.commit()
        if not users:
            LOGGER.warning("Authentication requires administrator bootstrap configuration.")
        else:
            LOGGER.info("Authentication ready using persisted credentials.")
        return result

    try:
        username, display_name, password = validate_admin_values(
            settings.admin_username,
            settings.admin_password,
            settings.admin_display_name,
        )
    except ValueError as exc:
        result = _configuration_error(db, "configuration_error", str(exc))
        ...
        try:
            with db.begin_nested():
                ...
                result = AuthInitResult(
                    ...
                    "bootstrap",
                    "Initial administrator created from Home Assistant configuration.",
                    user.id,
                    user.username,
                )
                _persist_result(db, result)
    ...
    if settings.admin_recovery_mode:
        if configured_user is not None and not configured_user.is_admin:
            message = (
                "Administrator recovery could not apply because the configured username belongs "
                "to a non-administrator user."
            )
            result = _configuration_error(db, "recovery_blocked", message)
            LOGGER.error(
                "Administrator recovery blocked by non-admin username collision configured_username=%s",
                username,
            )
            return result
        ...
        if target is None:
            message = (
                "Administrator recovery target is ambiguous. Configure an existing administrator "
                "username or ensure exactly one administrator exists before recovery."
            )
            result = _configuration_error(db, "recovery_blocked", message)
            LOGGER.error(
                "Administrator recovery blocked: ambiguous target configured_username=%s admin_count=%s",
                username,
                len(admins),
            )
            return result
        ...
        if collision is not None:
            ...
            result = _configuration_error(db, "recovery_blocked", message)
            LOGGER.error(
                "Administrator recovery blocked by username collision configured_username=%s",
                username,
            )
            return result
        ...
        try:
            with db.begin_nested():
                _set_app_config(db, AUTH_LAST_STATE_KEY, AuthLifecycleState.RECOVERY_REQUIRED.value)
            ...
            db.rollback()
            LOGGER.exception(
                "Administrator recovery failed and was rolled back username=%s",
                username,
            )
            raise
        LOGGER.info(
            "Administrator recovery completed successfully username=%s sessions_revoked=%s failed_attempts_cleared=%s",
            username,
            revoked,
            cleared,
        )
    ...
    if exact_admin is not None and stored_fingerprint == fingerprint:
        result = AuthInitResult(
            AuthLifecycleState.READY,
            "none",
            "Configured administrator credentials are already applied.",
            exact_admin.id,
            exact_admin.username,
        )
        with db.begin_nested():
            _persist_result(db, result)
        db.commit()
        LOGGER.info(
            "Authentication ready configured identity matches persisted administrator username=%s",
            username,
        )
        return result
            ...
            try:
                with db.begin_nested():
                    ...
                    result = AuthInitResult(
                        AuthLifecycleState.READY,
                        "legacy_adoption",
                        "Existing administrator adopted the configured Home Assistant credentials during legacy migration.",
                        target.id,
                        username,
                        revoked,
                        cleared,
                ...
                db.rollback()
                LOGGER.exception(
                    "Legacy administrator credential adoption failed and was rolled back username=%s",
                    username,
                )
                raise
    ...
    message = (
        "Persisted administrator credentials remain authoritative. Enable admin_recovery_mode for "
        "one restart to intentionally apply changed Home Assistant credentials."
    )
    result = AuthInitResult(
        AuthLifecycleState.READY_WITH_CONFIGURATION_WARNING,
        "warning",
        message,
        admins[0].id if len(admins) == 1 else None,
        admins[0].username if len(admins) == 1 else None,
    )
    with db.begin_nested():
        _persist_result(db, result)
    db.commit()
    LOGGER.warning(
        "Authentication ready with configuration warning configured_username=%s admin_count=%s",
        username,
        len(admins),
    )
    return result
...
def public_auth_status(db: DbSession) -> dict[str, object]:
    ...
    return {
        "authentication": "ready"
        if state in {AuthLifecycleState.READY, AuthLifecycleState.READY_WITH_CONFIGURATION_WARNING}
        else "not_ready",
...
def admin_auth_diagnostics(
    ...
    return {
        ...
        "configured_identity_match": bool(
            admin and configured_username and admin.username == configured_username
        ),
        "last_bootstrap_or_recovery": _app_config_value(db, AUTH_RECOVERY_KEY)
        or _app_config_value(db, CONFIG_APPLIED_KEY),
