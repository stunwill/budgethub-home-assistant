...
CATEGORY_SEED: dict[str, list[str]] = {
    "Housing": ["Mortgage", "Rent", "Council Rates", "Body Corporate", "Home Maintenance", "Home Improvements", "Security"],
...
class RecurringExpenseCreateV1(BaseModel):
    ...
    # Compatibility fields accepted from v0.x clients/imports.
...
def run_v1_migrations(engine: Engine) -> None:
    ...
    with engine.begin() as connection:
        ...
        # Preserve legacy account links. Only structured direct_debit=True is strong enough
        # to infer Direct Debit. The mere presence of an account must not invent behaviour.
        connection.execute(text("""
...
def _fetch_categories(db: DbSession, user: User) -> list[dict[str, Any]]:
    return [dict(row) for row in db.execute(
        text("SELECT * FROM categories WHERE user_id=:uid ORDER BY COALESCE(parent_id,id), name"),
        {"uid": user.id},
    ).mappings().all()]
...
def _category_exists(db: DbSession, user: User, category_id: int | None) -> bool:
    ...
    return bool(db.execute(
        text("SELECT id FROM categories WHERE id=:id AND user_id=:uid"),
...
def _would_cycle(db: DbSession, user: User, category_id: int, parent_id: int | None) -> bool:
    ...
    while current:
        ...
        seen.add(current)
        current = db.execute(
            text("SELECT parent_id FROM categories WHERE id=:id AND user_id=:uid"),
            {"id": current, "uid": user.id},
        ).scalar()
    return False


def _sync_category_denormalized_values(db: DbSession, user: User) -> None:
    """Keep legacy string presentation columns aligned while IDs remain authoritative."""
    ...
    for item in rows:
        ...
        for table in ("transactions", "income_sources", "recurring_expenses", "planned_spending"):
            if _has_column(db.connection(), table, "category_id"):
                db.execute(
                    text(f"UPDATE {table} SET category=:path WHERE user_id=:uid AND category_id=:cid"),
                    {"path": path, "uid": user.id, "cid": cid},
                )
        if _has_column(db.connection(), "bills", "category_id"):
            db.execute(
                text("UPDATE bills SET bill_type=:path WHERE user_id=:uid AND category_id=:cid"),
                {"path": path, "uid": user.id, "cid": cid},
            )
        db.execute(
            text("UPDATE budgets SET category_name=:path WHERE user_id=:uid AND category_id=:cid"),
            {"path": path, "uid": user.id, "cid": cid},
        )
...
def create_category_v1(db: DbSession, user: User, payload: dict[str, Any]) -> dict:
    ...
    """), {
        "uid": user.id, "name": name, "parent_id": parent_id,
        "icon": payload.get("icon"), "color": payload.get("color"),
        "category_type": payload.get("category_type") or "expense",
        "relationship": payload.get("budget_relationship") or "independent",
...
def update_category_v1(db: DbSession, user: User, category_id: int, payload: dict[str, Any]) -> dict:
    ...
    """), {
        "id": category_id, "uid": user.id, "name": name, "parent_id": parent_id,
        "icon": payload.get("icon", existing["icon"]), "color": payload.get("color", existing["color"]),
        "category_type": payload.get("category_type", existing["category_type"]),
        "relationship": relationship, "active": bool(payload.get("is_active", existing["is_active"])),
        "notes": payload.get("notes", existing["notes"]), "now": utcnow(),
    })
...
def ensure_reference_data(db: DbSession, user: User) -> None:
    ...
    for parent_name, children in CATEGORY_SEED.items():
        ...
        current_children = {
            _norm(row["name"]) for row in _fetch_categories(db, user)
    ...
    existing_types = {
        ...
        for row in db.execute(text("SELECT id,name FROM expense_types WHERE user_id=:uid"), {"uid": user.id}).all()
    }
    for name, description in EXPENSE_TYPE_SEED:
        if _norm(name) in existing_types:
            continue
        db.execute(text("""
            INSERT INTO expense_types(user_id,name,description,is_active,created_at,updated_at)
            VALUES(:uid,:name,:description,1,:now,:now)
        """), {"uid": user.id, "name": name, "description": description, "now": now})

    db.flush()

    # Convert legacy string references only when the match is unambiguous.
    for table, value_column in (
        ...
        rows = db.execute(text(
            f"SELECT id,{value_column} AS value FROM {table} WHERE user_id=:uid AND category_id IS NULL AND {value_column} IS NOT NULL"
        ), {"uid": user.id}).mappings().all()
        for row in rows:
            cid = _find_category_for_legacy_value(db, user, row["value"])
            if cid:
                db.execute(text(f"UPDATE {table} SET category_id=:cid WHERE id=:id AND user_id=:uid"),
                           {"cid": cid, "id": row["id"], "uid": user.id})
    ...
    for row in db.execute(text("""
        SELECT id,expense_type FROM recurring_expenses
        WHERE user_id=:uid AND expense_type_id IS NULL AND expense_type IS NOT NULL
    """), {"uid": user.id}).mappings().all():
        etid = type_by_name.get(_norm(row["expense_type"]))
        if etid:
            db.execute(text("UPDATE recurring_expenses SET expense_type_id=:etid WHERE id=:id AND user_id=:uid"),
                       {"etid": etid, "id": row["id"], "uid": user.id})
...
def list_expense_types(db: DbSession, user: User, include_inactive: bool = True) -> list[dict]:
    ...
    return [
        ...
        for row in db.execute(text(sql), {"uid": user.id}).mappings().all()
    ]
...
def create_expense_type(payload: dict[str, Any], current_user: User = USER, db: DbSession = DB):
    ...
    if db.execute(text("SELECT id FROM expense_types WHERE user_id=:uid AND lower(name)=lower(:name)"),
                  {"uid": current_user.id, "name": name}).scalar():
        raise HTTPException(status_code=409, detail="Expense Type already exists")
    now = utcnow()
    db.execute(text("""
        INSERT INTO expense_types(user_id,name,description,is_active,created_at,updated_at)
        VALUES(:uid,:name,:description,1,:now,:now)
    """), {"uid": current_user.id, "name": name, "description": payload.get("description"), "now": now})
...
def update_expense_type(expense_type_id: int, payload: dict[str, Any], current_user: User = USER, db: DbSession = DB):
    row = db.execute(text("SELECT * FROM expense_types WHERE id=:id AND user_id=:uid"),
                     {"id": expense_type_id, "uid": current_user.id}).mappings().first()
    if not row:
    ...
    if not name:
        raise HTTPException(status_code=400, detail="Expense Type name is required")
    duplicate = db.execute(text("""
        SELECT id FROM expense_types WHERE user_id=:uid AND id<>:id AND lower(name)=lower(:name)
    """), {"uid": current_user.id, "id": expense_type_id, "name": name}).scalar()
    if duplicate:
        raise HTTPException(status_code=409, detail="Expense Type already exists")
    db.execute(text("""
        UPDATE expense_types SET name=:name,description=:description,is_active=:active,
            archived_at=:archived,updated_at=:now
        WHERE id=:id AND user_id=:uid
    """), {
        "id": expense_type_id, "uid": current_user.id, "name": name,
        "description": payload.get("description", row["description"]),
        "active": bool(payload.get("is_active", row["is_active"])),
        "archived": None if bool(payload.get("is_active", row["is_active"])) else (row["archived_at"] or utcnow()),
        "now": utcnow(),
    })
    # Preserve IDs while keeping the v0.x display string current.
    db.execute(text("""
        UPDATE recurring_expenses SET expense_type=:name
        WHERE user_id=:uid AND expense_type_id=:id
    """), {"name": name, "uid": current_user.id, "id": expense_type_id})
    db.commit()
...
def _card_response(row) -> dict:
    return {
        ...
        "name": row.name, "card_type": row.card_type, "last_four": row.last_four,
        "display_name": f"{row.name} ••••{row.last_four}",
...
def _validate_card(db: DbSession, user: User, payload: dict[str, Any], existing=None) -> tuple[int, str, str, str, bool]:
    ...
    account = db.execute(text("SELECT id,is_active FROM accounts WHERE id=:id AND user_id=:uid"),
                         {"id": account_id, "uid": user.id}).first()
    if not account:
...
def create_card(payload: dict[str, Any], current_user: User = USER, db: DbSession = DB):
    ...
    now = utcnow()
    db.execute(text("""
        INSERT INTO cards(user_id,account_id,name,card_type,last_four,is_active,created_at,updated_at)
        VALUES(:uid,:account_id,:name,:card_type,:last_four,:active,:now,:now)
    """), {"uid": current_user.id, "account_id": account_id, "name": name, "card_type": card_type,
           "last_four": last_four, "active": active, "now": now})
    cid = int(db.execute(text("SELECT last_insert_rowid()")).scalar())
    db.commit()
...
def update_card(card_id: int, payload: dict[str, Any], current_user: User = USER, db: DbSession = DB):
    existing = db.execute(text("SELECT * FROM cards WHERE id=:id AND user_id=:uid"),
                          {"id": card_id, "uid": current_user.id}).first()
    if not existing:
        raise HTTPException(status_code=404, detail="Card not found")
    account_id, name, card_type, last_four, active = _validate_card(db, current_user, payload, existing)
    db.execute(text("""
        UPDATE cards SET account_id=:account_id,name=:name,card_type=:card_type,last_four=:last_four,
            is_active=:active,archived_at=:archived,updated_at=:now
        WHERE id=:id AND user_id=:uid
    """), {
        "id": card_id, "uid": current_user.id, "account_id": account_id, "name": name,
        "card_type": card_type, "last_four": last_four, "active": active,
        "archived": None if active else (existing.archived_at or utcnow()), "now": utcnow(),
    })
    db.commit()
...
def _get_expense_type(db: DbSession, user: User, expense_type_id: int | None, active_only: bool = False) -> dict | None:
    if expense_type_id is None:
        return None
    row = db.execute(text("SELECT * FROM expense_types WHERE id=:id AND user_id=:uid"),
                     {"id": expense_type_id, "uid": user.id}).mappings().first()
    if not row:
        raise HTTPException(status_code=404, detail="Expense Type not found")
...
def _get_card(db: DbSession, user: User, card_id: int | None, active_only: bool = False):
    if card_id is None:
        return None
    row = db.execute(text("""
        SELECT c.*,a.name AS account_name,a.is_active AS account_active
        FROM cards c JOIN accounts a ON a.id=c.account_id
        WHERE c.id=:id AND c.user_id=:uid
    """), {"id": card_id, "uid": user.id}).first()
    if not row:
        raise HTTPException(status_code=404, detail="Card not found")
...
def _validate_account(db: DbSession, user: User, account_id: int | None, active_only: bool = False) -> None:
    if account_id is None:
        return
    row = db.execute(text("SELECT is_active FROM accounts WHERE id=:id AND user_id=:uid"),
                     {"id": account_id, "uid": user.id}).first()
    if not row:
        raise HTTPException(status_code=404, detail="Account not found")
    if active_only and not bool(row.is_active):
...
def _annual_factor(frequency: str | None, interval_count: int | None = None) -> Decimal | None:
    return {
        ...
        "every_28_days": Decimal(13),
        "every_4_weeks": Decimal(13),
        "monthly": Decimal(12),
        "quarterly": Decimal(4),
        "yearly": Decimal(1),
    }.get(frequency) if frequency != "custom" else (
        Decimal(365) / Decimal(max(interval_count or 1, 1))
    )
...
def recurring_cost(amount_cents: int | None, frequency: str | None, interval_count: int | None = None) -> dict:
    ...
    factor = _annual_factor(frequency, interval_count)
    if factor is None:
        return {"monthly": None, "annual": None, "show_monthly": False}
    annual_cents = int((Decimal(abs(amount_cents)) * factor).quantize(Decimal("1"), rounding=ROUND_HALF_UP))
    show_monthly = frequency in {"weekly", "fortnightly", "every_28_days", "every_4_weeks", "monthly"} or (
        frequency == "custom" and (interval_count or 1) <= 31
    )
    monthly_cents = int((Decimal(annual_cents) / Decimal(12)).quantize(Decimal("1"), rounding=ROUND_HALF_UP)) if show_monthly else None
    return {
        "monthly": cents_to_decimal(monthly_cents) if monthly_cents is not None else None,
        "annual": cents_to_decimal(annual_cents),
        "show_monthly": show_monthly,
    }


def _row_namespace(row) -> Any:
...
def recurring_response(row) -> dict:
    """Compatibility response used by legacy imports. Rich list/create responses use _recurring_response."""
    row = _row_namespace(row)
    amount = cents_to_decimal(row.amount_cents) if getattr(row, "amount_cents", None) is not None else None
    pm = getattr(row, "payment_method", None) or ("direct_debit" if getattr(row, "direct_debit", False) else "not_set")
    missing = []
    if getattr(row, "amount_cents", None) is None:
        missing.append("amount")
    if not getattr(row, "frequency", None):
        missing.append("frequency")
    if not getattr(row, "next_due_date", None):
        missing.append("next date")
    if pm == "direct_debit" and getattr(row, "account_id", None) is None:
        missing.append("bank account")
    if pm == "automatic_card_payment" and getattr(row, "card_id", None) is None:
        missing.append("card")
    active = bool(getattr(row, "is_active", True))
    state = "complete" if not missing and active else "inactive" if not missing else "incomplete" if active else "inactive_incomplete"
    costs = recurring_cost(getattr(row, "amount_cents", None), getattr(row, "frequency", None), getattr(row, "interval_count", None))
    return {
        "id": row.id, "name": row.name, "amount": amount,
        "frequency": getattr(row, "frequency", None),
        "frequency_label": FREQUENCY_LABELS.get(getattr(row, "frequency", None), getattr(row, "frequency", None)),
        "interval_count": getattr(row, "interval_count", None),
        "next_due_date": str(getattr(row, "next_due_date", "") or "")[:10] or None,
        "payment_method": pm, "payment_method_label": PAYMENT_METHODS.get(pm, pm),
        "direct_debit": pm == "direct_debit",
        "account_id": getattr(row, "account_id", None), "card_id": getattr(row, "card_id", None),
        "source_account_text": getattr(row, "source_account_text", None),
        "category_id": getattr(row, "category_id", None), "category": getattr(row, "category", None),
        "expense_type_id": getattr(row, "expense_type_id", None), "expense_type": getattr(row, "expense_type", None),
        "payee_merchant": getattr(row, "payee_merchant", None),
        "amount_type": getattr(row, "amount_type", None) or ("variable_estimated" if getattr(row, "variable_amount", False) else "fixed"),
        "amount_type_label": AMOUNT_TYPES.get(getattr(row, "amount_type", None) or "fixed", "Fixed Amount"),
        "end_date": str(getattr(row, "end_date", "") or "")[:10] or None,
        "reminder_days_before": getattr(row, "reminder_days_before", None),
        "is_active": active, "variable_amount": (getattr(row, "amount_type", None) == "variable_estimated") or bool(getattr(row, "variable_amount", False)),
        "notes": getattr(row, "notes", None), "completeness": state, "missing_fields": missing,
        "calculated_cost": costs,
    }


def _recurring_response(db: DbSession, user: User, row) -> dict:
    base = recurring_response(row)
    category = _get_category(db, user, base["category_id"]) if base["category_id"] else None
    expense_type = _get_expense_type(db, user, base["expense_type_id"]) if base["expense_type_id"] else None
    card = _get_card(db, user, base["card_id"]) if base["card_id"] else None
    if category:
        base["category"] = category["path"]
        base["category_name"] = category["name"]
        base["category_active"] = category["is_active"]
    if expense_type:
        base["expense_type"] = expense_type["name"]
        base["expense_type_description"] = expense_type.get("description")
        base["expense_type_active"] = bool(expense_type.get("is_active"))
    if card:
        base["card"] = _card_response(card)
        # Card is the selected source. Account is contextual/derived, never separately required.
        base["derived_account_id"] = card.account_id
        base["derived_account_name"] = card.account_name
    else:
        base["card"] = None
        base["derived_account_id"] = None
        base["derived_account_name"] = None
    return base
...
def list_recurring_v1(db: DbSession, user: User, filter_value: str = "all") -> list[dict]:
    ...
    rows = db.execute(text("""
        ...
        WHERE user_id=:uid ORDER BY is_active DESC,next_due_date IS NULL,next_due_date,name
    """), {"uid": user.id}).all()
...
def _resolve_recurring_links(db: DbSession, user: User, payload: RecurringExpenseCreateV1) -> dict[str, Any]:
    ...
    # Backward-compatible structured direct-debit flag may promote only to direct debit.
    if payment_method == "not_set" and payload.direct_debit is True:
    ...
    if amount_type not in AMOUNT_TYPES:
        raise HTTPException(status_code=400, detail="Unsupported Amount Type")
    ...
    if category is None and payload.category:
        cid = _find_category_for_legacy_value(db, user, payload.category)
        category = _get_category(db, user, cid) if cid else None
    ...
    if expense_type is None and payload.expense_type:
        matches = [row for row in list_expense_types(db, user, False) if _norm(row["name"]) == _norm(payload.expense_type)]
        expense_type = matches[0] if len(matches) == 1 else None
    ...
    if payment_method == "direct_debit":
        if account_id is None:
            raise HTTPException(status_code=400, detail="Choose a Bank Account for Direct Debit")
        _validate_account(db, user, account_id, active_only=True)
        card_id = None
    elif payment_method == "automatic_card_payment":
        if card_id is None:
            raise HTTPException(status_code=400, detail="Choose a Card for Automatic Card Payment")
        card = _get_card(db, user, card_id, active_only=True)
        account_id = int(card.account_id)
    else:
        # Preserve a legacy Account relationship when supplied, but it is not a required
        # payment source for manual/BPAY/transfer/cash/other/not-set methods.
        _validate_account(db, user, account_id, active_only=False)
        card_id = None
    if payload.end_date and payload.next_due_date and payload.end_date < payload.next_due_date:
        raise HTTPException(status_code=400, detail="End Date cannot be before Next Due")
    return {
        "payment_method": payment_method, "amount_type": amount_type,
        "category": category, "expense_type": expense_type,
        "account_id": account_id, "card_id": card_id, "card": card,
    }


def create_recurring_v1(db: DbSession, user: User, payload: RecurringExpenseCreateV1) -> dict:
    links = _resolve_recurring_links(db, user, payload)
    now = utcnow()
    amount_cents = parse_money(payload.amount) if payload.amount not in (None, "") else None
    category = links["category"]
    expense_type = links["expense_type"]
    db.execute(text("""
        INSERT INTO recurring_expenses(
            user_id,name,amount_cents,frequency,interval_count,next_due_date,direct_debit,account_id,source_account_text,
            category,category_id,expense_type,expense_type_id,owner_group,is_active,variable_amount,aliases,notes,last_paid_date,
            payment_method,card_id,payee_merchant,amount_type,end_date,reminder_days_before,source,created_at,updated_at
        ) VALUES(
            :uid,:name,:amount,:frequency,:interval,:next_due,:direct_debit,:account_id,:source_account,
            :category,:category_id,:expense_type,:expense_type_id,:owner_group,:active,:variable,:aliases,:notes,:last_paid,
            :payment_method,:card_id,:payee,:amount_type,:end_date,:reminder,'manual',:now,:now
        )
    """), {
        "uid": user.id, "name": payload.name.strip(), "amount": amount_cents,
        "frequency": payload.frequency, "interval": payload.interval_count, "next_due": payload.next_due_date,
        "direct_debit": links["payment_method"] == "direct_debit", "account_id": links["account_id"],
        "source_account": payload.source_account_text,
        "category": category["path"] if category else payload.category,
        "category_id": category["id"] if category else None,
        "expense_type": expense_type["name"] if expense_type else payload.expense_type,
        "expense_type_id": expense_type["id"] if expense_type else None,
        "owner_group": payload.owner_group, "active": payload.is_active,
        "variable": links["amount_type"] == "variable_estimated", "aliases": payload.aliases,
        "notes": payload.notes, "last_paid": payload.last_paid_date,
        "payment_method": links["payment_method"], "card_id": links["card_id"],
        "payee": payload.payee_merchant, "amount_type": links["amount_type"],
        "end_date": payload.end_date, "reminder": payload.reminder_days_before, "now": now,
    })
    rid = int(db.execute(text("SELECT last_insert_rowid()")).scalar())
    db.commit()
    row = db.execute(text("SELECT * FROM recurring_expenses WHERE id=:id AND user_id=:uid"),
                     {"id": rid, "uid": user.id}).first()
    return _recurring_response(db, user, row)
...
def update_recurring_v1(expense_id: int, payload: dict[str, Any], current_user: User = USER, db: DbSession = DB):
    existing = db.execute(text("SELECT * FROM recurring_expenses WHERE id=:id AND user_id=:uid"),
                          {"id": expense_id, "uid": current_user.id}).mappings().first()
    if not existing:
        raise HTTPException(status_code=404, detail="Recurring Expense not found")
    merged = dict(existing)
    merged.update({k: v for k, v in payload.items() if v is not None})
    # Map persisted columns back to API field names.
    merged["amount"] = cents_to_decimal(existing["amount_cents"]) if existing["amount_cents"] is not None else None
    if "amount" in payload:
        merged["amount"] = payload["amount"]
    request = RecurringExpenseCreateV1(**{
        key: merged.get(key)
        for key in RecurringExpenseCreateV1.model_fields
        if key in merged
    })
    links = _resolve_recurring_links(db, current_user, request)

    effective_from = payload.get("effective_from")
    amount_update = payload.get("amount") if "amount" in payload else None
    values = {
        "id": expense_id, "uid": current_user.id, "name": request.name.strip(),
        "frequency": request.frequency, "interval": request.interval_count, "next_due": request.next_due_date,
        "direct_debit": links["payment_method"] == "direct_debit", "account_id": links["account_id"],
        "source_account": request.source_account_text,
        "category": links["category"]["path"] if links["category"] else request.category,
        "category_id": links["category"]["id"] if links["category"] else None,
        "expense_type": links["expense_type"]["name"] if links["expense_type"] else request.expense_type,
        "expense_type_id": links["expense_type"]["id"] if links["expense_type"] else None,
        "owner_group": request.owner_group, "active": request.is_active,
        "variable": links["amount_type"] == "variable_estimated", "aliases": request.aliases,
        "notes": request.notes, "last_paid": request.last_paid_date,
        "payment_method": links["payment_method"], "card_id": links["card_id"],
        "payee": request.payee_merchant, "amount_type": links["amount_type"],
        "end_date": request.end_date, "reminder": request.reminder_days_before, "now": utcnow(),
    }
    db.execute(text("""
        UPDATE recurring_expenses SET
            name=:name,frequency=:frequency,interval_count=:interval,next_due_date=:next_due,
            direct_debit=:direct_debit,account_id=:account_id,source_account_text=:source_account,
            category=:category,category_id=:category_id,expense_type=:expense_type,expense_type_id=:expense_type_id,
            owner_group=:owner_group,is_active=:active,variable_amount=:variable,aliases=:aliases,notes=:notes,
            last_paid_date=:last_paid,payment_method=:payment_method,card_id=:card_id,payee_merchant=:payee,
            amount_type=:amount_type,end_date=:end_date,reminder_days_before=:reminder,updated_at=:now
        WHERE id=:id AND user_id=:uid
    """), values)
    if "amount" in payload:
        new_amount = parse_money(amount_update) if amount_update not in (None, "") else None
        if effective_from and new_amount is not None:
            effective_date = date.fromisoformat(str(effective_from)[:10])
            # Future change: preserve the base/historical amount and add effective-dated configuration.
            db.execute(text("""
                INSERT INTO effective_amount_changes(
                    user_id,record_type,record_id,new_amount_cents,effective_from,source,notes,created_at,updated_at
                ) VALUES(:uid,'recurring_expense',:id,:amount,:effective_from,'edit',:notes,:now,:now)
            """), {"uid": current_user.id, "id": expense_id, "amount": new_amount,
                   "effective_from": effective_date, "notes": payload.get("edit_mode") or "Change going forward",
                   "now": utcnow()})
        else:
            db.execute(text("UPDATE recurring_expenses SET amount_cents=:amount WHERE id=:id AND user_id=:uid"),
                       {"amount": new_amount, "id": expense_id, "uid": current_user.id})
    db.commit()
    row = db.execute(text("SELECT * FROM recurring_expenses WHERE id=:id AND user_id=:uid"),
                     {"id": expense_id, "uid": current_user.id}).first()
    return _recurring_response(db, current_user, row)
...
def recurring_meta(current_user: User = USER, db: DbSession = DB):
    ensure_reference_data(db, current_user)
    return {
        "payment_methods": [{"value": key, "label": label} for key, label in PAYMENT_METHODS.items()],
        "amount_types": [{"value": key, "label": label} for key, label in AMOUNT_TYPES.items()],
        "frequencies": [{"value": key, "label": label} for key, label in FREQUENCY_LABELS.items()],
        "categories": [row for row in list_categories_v1(db, current_user) if row["is_active"]],
        "expense_types": list_expense_types(db, current_user, False),
        "cards": list_cards(db, current_user, False),
    }
...
def reference_data(current_user: User = USER, db: DbSession = DB):
    ...
    return {
        ...
        "expense_types": list_expense_types(db, current_user),
    }
...
def _effective_amount(db: DbSession, user: User, record_type: str, record_id: int, base: int | None, when: date) -> int | None:
    ...
    for change in db.execute(text("""
        SELECT * FROM effective_amount_changes
        WHERE user_id=:uid AND record_type=:kind AND record_id=:rid
          AND effective_from<=:when AND (effective_to IS NULL OR effective_to>=:when)
        ORDER BY effective_from,id
    """), {"uid": user.id, "kind": record_type, "rid": record_id, "when": when}).mappings().all():
...
def _occurrence_dates(start_value, range_start: date, range_end: date, frequency: str | None, interval: int | None, end_date=None):
    current = _as_date(start_value)
    hard_end = min(range_end, _as_date(end_date)) if _as_date(end_date) else range_end
    if not current or current > hard_end:
        return
...
def schedule_events_v1(db: DbSession, user: User, start: date, end: date) -> list[dict]:
    """Authoritative schedule with end dates and effective amounts, suppressing linked bill duplicates."""
    ensure_reference_data(db, user)
    events: list[dict] = []
    linked_bills = {
        (int(row.recurring_expense_id), str(row.due_date)[:10])
        for row in db.execute(text("""
    ...
    for row in db.execute(text("SELECT * FROM income_sources WHERE user_id=:uid AND is_active=1"), {"uid": user.id}).all():
        for when in _occurrence_dates(row.next_payment_date, start, end, row.frequency, row.interval_count, row.end_date):
            ...
            if amount is not None:
                events.append({
                    "date": when.isoformat(), "name": row.name, "amount_cents": amount,
                    "amount": cents_to_decimal(amount), "kind": "income",
                    "category": row.category or "Revenue / Income", "provider": row.payer, "source": "income",
                })
    for row in db.execute(text("SELECT * FROM recurring_expenses WHERE user_id=:uid AND is_active=1"), {"uid": user.id}).all():
        for when in _occurrence_dates(row.next_due_date, start, end, row.frequency, row.interval_count, getattr(row, "end_date", None)):
            if (int(row.id), when.isoformat()) in linked_bills:
                continue
            amount = _effective_amount(db, user, "recurring_expense", row.id, row.amount_cents, when)
            if amount is not None:
                events.append({
                    "date": when.isoformat(), "name": row.name, "amount_cents": amount,
                    "amount": cents_to_decimal(amount), "kind": "recurring_expense",
                    "category": row.category or "Miscellaneous",
                    "provider": getattr(row, "payee_merchant", None) or row.expense_type,
                    "account": row.source_account_text, "source": "recurring_expense",
                })
    for row in db.execute(text("""
        SELECT * FROM bills WHERE user_id=:uid AND is_active=1
          AND paid_at IS NULL AND resolved_at IS NULL
    """), {"uid": user.id}).all():
        due = _as_date(row.due_date)
        if due and row.remaining_amount_cents is not None and start <= due <= end:
            events.append({
                "date": due.isoformat(), "name": row.name, "amount_cents": row.remaining_amount_cents,
                "amount": cents_to_decimal(row.remaining_amount_cents), "kind": "bill",
                "category": row.bill_type or "Financial Obligations", "provider": row.provider,
                "source": "bill",
            })
    for row in db.execute(text("""
        SELECT * FROM planned_spending
        WHERE user_id=:uid AND include_in_forecast=1 AND archived_at IS NULL
          AND status IN ('planned','committed')
    """), {"uid": user.id}).all():
        when = _as_date(row.planned_date)
        if when and row.estimated_amount_cents is not None and start <= when <= end:
            events.append({
                "date": when.isoformat(), "name": row.name, "amount_cents": row.estimated_amount_cents,
                "amount": cents_to_decimal(row.estimated_amount_cents), "kind": "planned_spending",
                "category": row.category or "Planned Spending", "provider": row.merchant,
                "status": row.status, "priority": row.priority, "source": "planned_spending",
            })
...
def forecast_recurring_events_v1(db: DbSession, user: User, start: date, end: date, scenario: dict | None = None) -> list[dict]:
    """Forecast recurring layer matching schedule semantics, including end dates and effective changes."""
    ...
    events = []
    for record_type, table, date_col, account_col, direction, source_type in [
        ("income", "income_sources", "next_payment_date", "destination_account_id", "income", "income"),
        ("recurring_expense", "recurring_expenses", "next_due_date", "account_id", "expense", "recurring_expense"),
    ]:
        rows = db.execute(text(
            f"SELECT * FROM {table} WHERE user_id=:uid AND is_active=1 AND {date_col} IS NOT NULL AND amount_cents IS NOT NULL"
        ), {"uid": user.id}).mappings().all()
        for row in rows:
            if record_type == "recurring_expense" and int(row["id"]) in removed:
                continue
            for when in _occurrence_dates(
                row[date_col], start, end, row["frequency"], row["interval_count"],
                row.get("end_date"),
            ):
                if record_type == "recurring_expense" and (int(row["id"]), when.isoformat()) in linked_bills:
                    continue
                amount = _effective_amount(db, user, record_type, int(row["id"]), row["amount_cents"], when)
                amount = virtual_changes.get(f"{record_type}:{row['id']}:{when.isoformat()}", amount)
                if amount is None:
                    continue
                explanation = "Recurring income" if direction == "income" else "Recurring expense"
                events.append(_event(
                    when, row["name"], int(amount), direction, source_type, int(row["id"]),
                    row["category"], row[account_col], "confirmed", "committed", explanation,
                ))
    return events


@router.get("/v1/acceptance/data-integrity")
def data_integrity(current_user: User = USER, db: DbSession = DB):
    """Small authenticated diagnostic used by release acceptance without exposing financial details."""
    ensure_reference_data(db, current_user)
    orphan_cards = db.execute(text("""
        SELECT COUNT(*) FROM cards c LEFT JOIN accounts a ON a.id=c.account_id
        WHERE c.user_id=:uid AND a.id IS NULL
    """), {"uid": current_user.id}).scalar()
    orphan_categories = 0
    for table in ("transactions", "income_sources", "recurring_expenses", "planned_spending"):
        orphan_categories += int(db.execute(text(f"""
            SELECT COUNT(*) FROM {table} x LEFT JOIN categories c ON c.id=x.category_id
            WHERE x.user_id=:uid AND x.category_id IS NOT NULL AND c.id IS NULL
        """), {"uid": current_user.id}).scalar() or 0)
    return {
        "schema_version": int(db.execute(text("SELECT MAX(version) FROM schema_version")).scalar() or 0),
        "orphan_cards": int(orphan_cards or 0),
        "orphan_category_references": orphan_categories,
        "status": "ok" if not orphan_cards and not orphan_categories else "needs_review",
    }
