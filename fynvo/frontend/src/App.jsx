...
export default function App() {
  ...
  async function loadData() {
    ...
    const commandPromise = j(`/dashboard/command-centre?range_days=${rangeDays}`).then((command) => {
    ...
    ]).then(([accounts, transactions, income, recurring, bills, planned, categories, budgets, goals, imports, review, suggestions, insights, financialHealth, budgetAnalysis, forecast]) => {
    ...
    await Promise.allSettled([commandPromise, restPromise]);
  }
  useEffect(() => { loadAuth(); }, []);
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      document.removeEventListener('visibilitychange', syncGreeting);
    };
  }, []);
  useEffect(() => {
...
function Kpi({ icon, label, value, hint, loading = false }) { const formatted = money(value); return <article className={`kpi ${loading ? 'is-loading' : ''}`}><span className="kpi-icon">{icon}</span><div><span>{label}</span><strong className={!loading && value !== null && value !== undefined ? amountClass(value) : ''}>{loading || formatted === null ? <span className="loading-value" aria-label={`${label} loading`}>Loading…</span> : formatted}</strong>{hint && <small>{hint}</small>}</div></article>; }
...
function GoalMini({ goal }) { const pct = Number(goal.progress?.percentage || 0); return <div className="goal-mini"><div><strong>{goal.name}</strong><small>{goal.progress?.status || goal.calculated_status}</small></div><progress value={Math.min(pct, 100)} max="100"></progress><span>{money(goal.progress?.current) || 'Loading…'} of {money(goal.progress?.target) || 'Loading…'} • {pct}%</span></div>; }
...
function RecordTable({ active, data, onEdit }) {
  ...
  return <section className={`panel ${active === 'Accounts' ? 'accounts-panel' : ''}`}><div className="panel-head"><div><h2>{active}</h2><p className="muted">Create and maintain {active.toLowerCase()} using the current Fynvo data model.</p></div><button className="primary ghost" onClick={() => onEdit({ type, label: `New ${label}`, row: { id: null }, values: normaliseRecord(type, {}) })}>+ Add</button></div>{rows.length ? <div className="table"><div className="thead"><span>Date</span><span>Name</span><span>Amount</span><span>Status</span><span></span></div>{rows.map((row) => <div className="tr" key={row.id}><span>{dateLabel(row.date || row.transaction_date || row.due_date || row.next_due_date || row.next_payment_date || row.planned_date)}</span><span>{row.name || row.description || row.merchant || row.category || row.account_type}</span><span className={amountClass(row.amount || row.estimated_amount || row.current_balance)}>{money(row.amount || row.estimated_amount || row.current_balance) || 'Loading…'}</span><span>{active === 'Accounts' ? accountTypeLabel(row.account_type) : row.status || row.completeness || row.category_type || 'Active'}</span><button onClick={() => onEdit({ type, label, row, values: normaliseRecord(type, row) })}>Edit</button></div>)}</div> : <Empty title={`No ${active.toLowerCase()} yet`}>Use Add to create the first record.</Empty>}</section>;
}

function Budgeting({ budgets, analysis, onEdit }) { return <section className="panel"><PanelHead title="Budgeting"/><div className="summary-row"><span>Base budget</span><strong>{money(analysis?.summary?.base_budget) || 'Loading…'}</strong></div><div className="summary-row"><span>Actual spending</span><strong>{money(analysis?.summary?.actual) || 'Loading…'}</strong></div><div className="summary-row"><span>Forecast spending</span><strong>{money(analysis?.summary?.forecast) || 'Loading…'}</strong></div><RecordList rows={budgets} onEdit={onEdit}/></section>; }
function RecordList({ rows, onEdit }) { return rows?.length ? rows.map((row) => <div className="list-row" key={row.id}><span>{row.name}<small>{row.period}</small></span><strong>{money(row.amount) || 'Loading…'}</strong><button onClick={() => onEdit(row)}>Edit</button></div>) : <Empty title="No records yet">Create a record to get started.</Empty>; }
...
function DynamicFields({ type, values, set, data }) {
  ...
  const categoryOptions = (data.categories || []).filter((category) => category.is_active !== false && (!category.category_type || category.category_type === 'expense')).slice().sort((a, b) => String(a.name || '').localeCompare(String(b.name || '')));
  ...
  if (type === 'planned') return <div className="form-grid planned-form">{text('name', 'Name', 'text', true, 'wide')}<div className="planned-primary-row">{text('estimated_amount', 'Estimated amount')}{text('planned_date', 'Planned date', 'date')}</div><Field label="Category"><select value={values.category || ''} onChange={(e) => set('category', e.target.value)}><option value="">Choose category</option>{categoryOptions.map((category) => <option key={category.id || category.name} value={category.name}>{category.name}</option>)}</select></Field><Field label="Status"><select value={values.status} onChange={(e) => set('status', e.target.value)}><option value="wishlist">Wishlist</option><option value="planned">Planned</option><option value="committed">Committed</option><option value="purchased">Purchased</option><option value="cancelled">Cancelled</option></select></Field>{text('description', 'Description', 'text', false, 'wide')}{text('notes', 'Notes', 'text', false, 'wide')}</div>;
