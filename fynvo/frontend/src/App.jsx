...
const dateLabel = (value) => value ? new Intl.DateTimeFormat('en-AU', { day: '2-digit', month: 'short', year: 'numeric' }).format(new Date(value)) : 'No date';
...
function friendlyError(payload, fallback) {
  ...
  if (Array.isArray(detail)) return detail.map((item) => `${item.loc?.slice(-1)[0] || 'Field'}: ${item.msg}`).join(' ');
