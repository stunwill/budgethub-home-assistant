...
export function RecurringPaymentFieldsV17({ values, set, data }) {
  ...
  return <fieldset className="payment-v17-section"><legend>Payment</legend>
    ...
    <label className="field"><span>Payment Method</span><select value={method} onChange={(event) => setMethod(event.target.value)}>
      {Object.entries(methodLabels).map(([id, label]) => <option key={id} value={id}>{label}</option>)}
    </select></label>
    ...
    {method === 'automatic_card_payment' && <div className="payment-v17-card-field"><label className="field"><span>Card</span><select required value={values.card_id || ''} onChange={(event) => set('card_id', event.target.value)}><option value="">{(data.cards || []).length ? 'Choose Card' : 'No Cards available'}</option>{(data.cards || []).filter((card) => card.is_active !== false).map((card) => <option key={card.id} value={card.id}>{card.display_name}</option>)}</select></label>{selectedCard && <small className="payment-v17-linked">Linked to account: <strong>{selectedCard.account_name}</strong></small>}</div>}
...
export function CardsPageV17({ accounts, cards, onRefresh }) {
  ...
  const start = (card = null, accountId = '') => setEdit({ id: card?.id || null, name: card?.name || '', account_id: card?.account_id || accountId || accounts[0]?.id || '', card_type: card?.card_type || 'debit', last_four: card?.last_four || '', is_active: card?.is_active ?? true });
  ...
  return <section className="panel cards-v17-page"><div className="panel-head"><div><h2>Cards</h2><p className="muted">Cards belong to an Account. Fynvo stores only the last four digits.</p></div><button className="primary ghost" onClick={() => start()}>+ Add Card</button></div>
    <div className="cards-v17-groups">{grouped.map(({ account, cards: accountCards }) => <article key={account.id} className="cards-v17-account"><div><strong>{account.name}</strong><small>{account.institution || account.account_type}</small></div><div className="cards-v17-list">{accountCards.length ? accountCards.map((card) => <button key={card.id} type="button" className="cards-v17-card" onClick={() => start(card)}><span><strong>{card.display_name}</strong><small>{card.card_type} · {card.is_active ? 'Active' : 'Inactive'}</small></span><small>Linked account: {card.account_name}</small></button>) : <p className="muted">No Cards linked to this Account.</p>}<button type="button" className="link-button" onClick={() => start(null, account.id)}>+ Add Card</button></div></article>)}</div>
    {edit && <div className="modal-backdrop"><form className="modal" onSubmit={save}><div className="panel-head"><h2>{edit.id ? 'Edit Card' : 'Add Card'}</h2><button type="button" onClick={() => setEdit(null)}>×</button></div><div className="form-grid"><label className="field"><span>Card Name</span><input required value={edit.name} onChange={(event) => setEdit({ ...edit, name: event.target.value })}/></label><label className="field"><span>Linked Account</span><select required value={edit.account_id} onChange={(event) => setEdit({ ...edit, account_id: event.target.value })}>{accounts.filter((account) => account.is_active !== false).map((account) => <option key={account.id} value={account.id}>{account.name}</option>)}</select></label><label className="field"><span>Card Type</span><select value={edit.card_type} onChange={(event) => setEdit({ ...edit, card_type: event.target.value })}><option value="debit">Debit</option><option value="credit">Credit</option><option value="prepaid">Prepaid</option><option value="other">Other</option></select></label><label className="field"><span>Last 4 Digits</span><input required inputMode="numeric" pattern="[0-9]{4}" maxLength="4" value={edit.last_four} onChange={(event) => setEdit({ ...edit, last_four: event.target.value.replace(/\D/g, '').slice(0, 4) })}/></label>{edit.id && <label className="field checkbox"><input type="checkbox" checked={edit.is_active} onChange={(event) => setEdit({ ...edit, is_active: event.target.checked })}/><span>Active</span></label>}</div>{error && <p className="error">{error}</p>}<div className="modal-actions"><button type="button" onClick={() => setEdit(null)}>Cancel</button><button className="primary">Save Card</button></div></form></div>}
  </section>;
}
...
export function PaymentReconciliationV17({ money, dateLabel }) {
  ...
  return <section className="panel payments-v17-review"><div className="panel-head"><div><h2>Scheduled Payment Review</h2><p className="muted">Review imported transactions that may match expected recurring payments.</p></div></div>{rows.length ? <div className="table"><div className="thead"><span>Imported transaction</span><span>Possible match</span><span>Confidence</span><span>Difference</span><span></span></div>{rows.map((row) => <div className="tr" key={`${row.transaction_id}-${row.scheduled_payment_id}`}><span>{row.transaction_description}<small>{dateLabel(row.transaction_date)} · {money(row.transaction_amount)}</small></span><span>{row.expense_name}<small>Expected {dateLabel(row.expected_date)} · {money(row.expected_amount)}</small></span><span><span className={`payment-v17-confidence ${row.confidence}`}>{row.confidence}</span></span><span>{money(row.difference)}</span><button type="button" className="primary ghost" onClick={() => confirm(row)}>Confirm match</button></div>)}</div> : <p className="muted">No suggested scheduled-payment matches currently need review.</p>}</section>;
}
