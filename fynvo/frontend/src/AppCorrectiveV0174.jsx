...
function forecastSource(event, data) {
  const mapping = { income: ['income', data.income], recurring_expense: ['recurring', data.recurring], bill: ['bills', data.bills], planned_spending: ['planned', data.planned] };
...
export default function AppCorrectiveV0174() {
  ...
  async function refreshDashboard(days = rangeDays) {
    const requestId = ++commandRequestRef.current;
    setDashboardLoading(true);
    try {
      const command = await j(`/dashboard/command-centre?range_days=${days}`);
      if (requestId !== commandRequestRef.current) return;
      setData((current) => ({ ...current, command }));
    } finally {
      if (requestId === commandRequestRef.current) setDashboardLoading(false);
    }
  }
  async function loadSupportingData() {
    const [accounts, transactions, income, recurring, bills, planned, categories, budgets, goals, imports, review, suggestions, insights, financialHealth, budgetAnalysis, forecast] = await Promise.all([
      j('/accounts'), j('/transactions'), j('/income'), j('/recurring-expenses'), j('/bills'), j('/planned-spending'), j('/categories'), j('/budgets'), j('/goals'), j('/imports/history'), j('/reconciliation/review-queue'), j('/intelligence/suggestions'), j(`/insights?horizon_days=${rangeDays}&refresh=false`), j(`/insights/financial-health?horizon_days=${rangeDays}`), j('/budgets/analysis'), j(`/forecast?mode=expected&horizon=${rangeDays}d`),
    ]);
    setData((current) => ({ ...current, accounts: accounts || [], transactions: transactions || [], income: income || [], recurring: recurring || [], bills: bills || [], planned: planned || [], categories: categories || [], budgets: budgets || [], goals: goals || [], imports: imports || [], review: review || [], suggestions: suggestions || [], insights: insights || [], financialHealth, budgetAnalysis, forecast }));
  }
  async function loadAll() {
    await Promise.allSettled([refreshDashboard(rangeDays), loadSupportingData()]);
  }
  ...
  useEffect(() => {
    const syncGreeting = () => setGreeting(greetingForNow());
    syncGreeting();
    const timer = window.setInterval(syncGreeting, 60000);
    window.addEventListener('focus', syncGreeting);
    document.addEventListener('visibilitychange', syncGreeting);
    return () => { window.clearInterval(timer); window.removeEventListener('focus', syncGreeting); document.removeEventListener('visibilitychange', syncGreeting); };
  }, []);
  useEffect(() => {
    const media = window.matchMedia('(max-width: 980px)');
    const sync = (event) => { setIsMobile(event.matches); setMobileNavOpen(false); document.body.style.overflow = ''; };
  ...
  async function submitAuth(e) { e.preventDefault(); setError(''); const payload = auth?.setup_required ? { username: form.username, display_name: form.display_name || form.username, password: form.password } : { username: form.username, password: form.password }; const res = await api(auth?.setup_required ? '/auth/setup' : '/auth/login', { method: 'POST', body: JSON.stringify(payload) }); if (!res.ok) { const p = await res.json().catch(() => ({})); setError(p.detail || 'Sign in failed'); return; } await loadAuth(); }
  async function logout() { await api('/auth/logout', { method: 'POST' }); setAuth({ ...auth, authenticated: false }); }
  async function saveEdit(e) { e.preventDefault(); setError(''); setSuccess(''); if (!edit) return; const creating = !edit.row?.id; const path = creating ? createPath(edit.type) : endpointFor(edit.type, edit.row.id); const payload = { ...edit.values }; if (payload.account_id === '') payload.account_id = null; if (payload.destination_account_id === '') payload.destination_account_id = null; const res = await api(path, { method: creating ? 'POST' : 'PUT', body: JSON.stringify(payload) }); if (!res.ok) { setError(friendlyError(await res.json().catch(() => null), `Could not ${creating ? 'create' : 'save'} ${edit.label}. Check the fields and try again.`)); return; } const message = `${creating ? edit.label.replace(/^New /, '') + ' created.' : edit.label + ' updated.'}`; setEdit(null); setSuccess(message); await loadAll(); }
  ...
  return <div className={`shell ${mobileNavOpen ? 'mobile-nav-open' : ''}`}>
    ...
    <main className="content">
      ...
      {active === 'Overview' && <Overview data={data} setActive={navigate} rangeDays={rangeDays} setQuick={setQuick} quickDefaults={quickDefaults} loading={dashboardLoading}/>} 
...
function Overview({ data, setActive, rangeDays, setQuick, quickDefaults, loading }) {
  const command = data.command || {}; const kpis = command.kpis || {}; const forecast = command.forecast?.baseline; const expected = command.forecast?.expected; const planned = command.top_planned_spending || data.planned || []; const attention = command.attention || {};
  return <div className="dashboard-page"><section className="kpi-grid five"><Kpi icon="💵" label="Available Cash" value={kpis.available_cash} loading={loading}/><Kpi icon="📈" label="Expected Income" value={kpis.expected_income} hint={`${rangeDays} days`} loading={loading}/><Kpi icon="🧾" label="Scheduled Commitments" value={kpis.scheduled_commitments} hint={`${rangeDays} days`} loading={loading}/><Kpi icon="🛒" label="Planned Spending" value={kpis.planned_spending} hint={`${rangeDays} days`} loading={loading}/><Kpi icon="↗" label="Projected Balance" value={kpis.projected_balance} hint={`End of ${rangeDays} days`} loading={loading}/></section><section className="command-grid"><article className="panel chart-panel"><PanelHead title="Cash Flow Forecast" action="View full cash flow →" onAction={() => setActive('Cash Flow')}/><CashFlowChart baseline={forecast} expected={expected}/><ForecastMetrics forecast={forecast}/></article><article className="panel"><PanelHead title="Forecast Summary" meta={`End of ${rangeDays} days`}/><SummaryRow label="Baseline Forecast" value={command.forecast?.summary?.baseline}/><SummaryRow label="Expected Forecast" value={command.forecast?.summary?.expected}/>{command.forecast?.summary?.lowest_balance && <SummaryRow label="Lowest Balance" value={command.forecast.summary.lowest_balance.balance}/>}<button className="link-button" onClick={() => setActive('Cash Flow')}>View cash flow →</button></article><article className="panel"><PanelHead title="Upcoming Commitments" action="View calendar →" onAction={() => setActive('Calendar')}/><CompactEvents rows={command.upcoming_commitments || []}/></article></section><section className="card-grid"><article className="panel"><PanelHead title="Upcoming" meta="Next 7 days" action="View all →" onAction={() => setActive('Calendar')}/><CompactEvents rows={(command.upcoming || []).slice(0, 4)}/></article><article className="panel"><PanelHead title="Top Planned Spending" action="+ Quick Add" onAction={() => setQuick(quickDefaults('planned'))}/>{planned.length ? planned.slice(0, 4).map((item) => <div className="list-row" key={item.id || item.name}><span>{item.name}<small>{dateLabel(item.planned_date)}</small></span><strong>{money(item.estimated_amount) || 'Loading…'}</strong></div>) : <Empty title="No planned spending">No planned purchases during this period.</Empty>}</article><article className="panel attention"><PanelHead title="Financial Health" action="View insights →" onAction={() => setActive('Insights')}/><strong>{attention.headline || 'No major issues detected'}</strong></article></section></div>;
}
function Kpi({ icon, label, value, hint, loading = false }) { const formatted = money(value); return <article className={`kpi ${loading ? 'is-loading' : ''}`}><span className="kpi-icon">{icon}</span><div><span>{label}</span><strong className={!loading && value !== null && value !== undefined ? amountClass(value) : ''}>{loading || formatted === null ? <span className="loading-value">Loading…</span> : formatted}</strong>{hint && <small>{hint}</small>}</div></article>; }
...
function SummaryRow({ label, value }) { const formatted = money(value); return <div className="summary-row"><span>{label}</span><strong className={value !== null && value !== undefined ? amountClass(value) : ''}>{formatted || 'Loading…'}</strong></div>; }
function CashFlowChart({ baseline, expected }) { return <CashFlowChartV0174 baseline={baseline} expected={expected} money={money} dateLabel={dateLabel} Empty={Empty}/>; }
function ForecastMetrics({ forecast }) { if (!forecast) return null; return <div className="metric-strip"><div><span>End of period</span><strong>{money(forecast.final_balance) || 'Loading…'}</strong></div><div><span>Lowest Balance</span><strong className="negative">{money(forecast.lowest_balance?.balance) || 'Loading…'}</strong><small>{dateLabel(forecast.lowest_balance?.date)}</small></div>{forecast.shortfall ? <div className="risk"><span>Cash shortfall risk</span><strong>{money(forecast.shortfall.balance) || 'Loading…'}</strong><small>{dateLabel(forecast.shortfall.date)}</small></div> : <div className="safe"><span>No forecast shortfall</span><strong>Clear</strong><small>Based on current records</small></div>}</div>; }
function CompactEvents({ rows }) { return rows?.length ? rows.slice(0, 6).map((row, index) => <div className="event-row" key={`${row.date}-${row.name}-${index}`}><time>{dateLabel(row.date).slice(0, 6)}</time><span>{row.name}<small>{row.category || row.source_type || row.source}</small></span><strong className={amountClass(row.amount)}>{money(row.amount) || 'Loading…'}</strong></div>) : <Empty title="Nothing scheduled">No matching financial events in this period.</Empty>; }
function GoalsPage({ goals, accounts, onEdit, onAdd }) { const totals = goals.reduce((acc, goal) => { acc.target += parseAmount(goal.target_amount); acc.current += parseAmount(goal.progress?.current || goal.current_amount); if (goal.progress?.status === 'on_track' || goal.progress?.status === 'ahead') acc.onTrack += 1; return acc; }, { target: 0, current: 0, onTrack: 0 }); return <section className="stack"><div className="kpi-grid four"><Kpi icon="🎯" label="Active Goals" value={goals.length}/><Kpi icon="🏁" label="Total Target" value={totals.target}/><Kpi icon="💰" label="Allocated / Saved" value={totals.current}/><Kpi icon="✅" label="Goals On Track" value={`${totals.onTrack} of ${goals.length}`}/></div><article className="panel"><PanelHead title="Financial Goals" action="+ Add Goal" onAction={onAdd}/>{goals.map((goal) => <div className="list-row" key={goal.id}><span>{goal.name}</span><button onClick={() => onEdit(goal)}>Edit</button></div>)}</article></section>; }
function ForecastPage({ forecast, onView }) { const events = forecast?.events || []; return <section className="panel"><PanelHead title="Cash Flow"/><CashFlowChart baseline={forecast} expected={forecast}/><ForecastMetrics forecast={forecast}/><div className="cashflow-events"><h2>Upcoming cash flow</h2>{events.length ? <div className="table cashflow-table">{events.map((item, i) => <div className="tr" key={`${item.source_type}-${item.source_id || i}-${item.date}`}><span>{dateLabel(item.date)}</span><span className="event-name">{item.name}<small>{item.category || item.source_type?.replaceAll('_', ' ')}</small></span><span className={amountClass(item.amount)}>{money(item.amount) || 'Loading…'}</span><span>{money(item.forecast_balance) || 'Loading…'}</span><button type="button" className="event-action" onClick={() => onView(item)}>View more info</button></div>)}</div> : <Empty title="Nothing scheduled">No matching financial events in this period.</Empty>}</div></section>; }
...
function SpendingIntelligence({ suggestions, dismissSuggestion }) { return <section className="panel"><PanelHead title="Spending Intelligence"/>{suggestions?.length ? suggestions.map((item) => <div className="suggestion" key={item.id}><div><strong>{item.title || item.suggestion_type}</strong></div><Badge>{item.confidence || 'review'}</Badge><button onClick={() => dismissSuggestion(item.id)}>Dismiss</button></div>) : <Empty title="No suggestions waiting">Spending Intelligence has no unresolved items.</Empty>}</section>; }
function RecordTable({ active, data, onEdit }) { const cfg = { Transactions: ['transactions', data.transactions, 'Transaction'] }[active]; const [type, rows, label] = cfg; return <section className="panel"><div className="panel-head"><div><h2>{active}</h2></div><button className="primary ghost" onClick={() => onEdit({ type, label: `New ${label}`, row: { id: null }, values: normaliseRecord(type, {}) })}>+ Add</button></div>{rows.length ? <div className="table">{rows.map((row) => <div className="tr" key={row.id}><span>{dateLabel(row.date || row.transaction_date)}</span><span>{row.name || row.description || row.merchant || row.account_type}</span><span>{money(row.amount) || ''}</span><span>{row.status || 'Active'}</span><button onClick={() => onEdit({ type, label, row, values: normaliseRecord(type, row) })}>Edit</button></div>)}</div> : <Empty title={`No ${active.toLowerCase()} yet`}>Use Add to create the first record.</Empty>}</section>; }
function Budgeting({ budgets, analysis, onEdit }) { return <section className="panel"><PanelHead title="Budgeting"/><SummaryRow label="Base budget" value={analysis?.summary?.base_budget}/><SummaryRow label="Actual spending" value={analysis?.summary?.actual}/>{budgets.map((row) => <div className="list-row" key={row.id}><span>{row.name}</span><button onClick={() => onEdit(row)}>Edit</button></div>)}</section>; }
function CsvImport({ state, setState, accounts, previewImport, commitImport }) { return <section className="panel"><PanelHead title="CSV Import & Reconciliation"/><form className="form-grid" onSubmit={previewImport}><Field label="Filename" value={state.filename} onChange={(e) => setState({ ...state, filename: e.target.value })}/><Field label="Account"><select value={state.account_id} onChange={(e) => setState({ ...state, account_id: e.target.value })}><option value="">Choose account</option>{accounts.map((a) => <option key={a.id} value={a.id}>{a.name}</option>)}</select></Field><label className="field wide"><span>CSV text</span><textarea rows="10" value={state.csv_text} onChange={(e) => setState({ ...state, csv_text: e.target.value })}/></label><button className="primary">Preview CSV</button></form>{state.preview && <button className="primary" onClick={commitImport}>Commit Import</button>}</section>; }
function ImportHistory({ rows }) { return <section className="panel"><PanelHead title="Import History"/>{rows?.length ? rows.map((row) => <div className="list-row" key={row.id}><span>{row.filename}</span><strong>{row.imported_count} imported</strong></div>) : <Empty title="No imports yet">Import a CSV to see history.</Empty>}</section>; }
function ReviewQueue({ rows, acceptMatch }) { return <section className="panel"><PanelHead title="Reconciliation Review Queue"/>{rows?.length ? rows.map((row) => <div className="suggestion" key={row.id}><div><strong>{row.source_type}</strong></div><button onClick={() => acceptMatch(row.id)}>Accept</button></div>) : <Empty title="No reconciliation items">Imported transactions that need review will appear here.</Empty>}</section>; }
...
function DynamicFields({ type, values, set, data }) {
  ...
  if (type === 'recurring') return <div className="form-grid">{text('name', 'Name', 'text', true)}{text('amount', 'Amount')}{text('next_due_date', 'Next due', 'date')}<Field label="Frequency"><select value={values.frequency} onChange={(e) => set('frequency', e.target.value)}><option>weekly</option><option>fortnightly</option><option>every_4_weeks</option><option>monthly</option><option>quarterly</option><option>yearly</option></select></Field><Field label="Account">{accountSelect('account_id')}</Field><Field label="Category"><CategorySelect categories={data.categories} value={values.category || ''} onChange={(e) => set('category', e.target.value)}/></Field>{text('expense_type', 'Expense type')}{text('notes', 'Notes')}</div>;
  if (type === 'bills') return <div className="form-grid">{text('name', 'Name', 'text', true)}{text('amount', 'Amount')}{text('due_date', 'Due date', 'date')}{text('provider', 'Provider')}{text('bill_type', 'Bill type')}<Field label="Account">{accountSelect('account_id')}</Field></div>;
  if (type === 'planned') return <div className="form-grid planned-form">{text('name', 'Name', 'text', true, 'wide')}<div className="planned-half-row"><Field label="Estimated Amount"><input value={values.estimated_amount || ''} onChange={(e) => set('estimated_amount', e.target.value)}/></Field><Field label="Planned Date"><input type="date" value={values.planned_date || ''} onChange={(e) => set('planned_date', e.target.value)}/></Field></div><Field label="Category"><CategorySelect categories={data.categories} value={values.category || ''} onChange={(e) => set('category', e.target.value)}/></Field><Field label="Status"><select value={values.status || 'planned'} onChange={(e) => set('status', e.target.value)}><option value="wishlist">Wishlist</option><option value="planned">Planned</option><option value="committed">Committed</option><option value="purchased">Purchased</option><option value="cancelled">Cancelled</option></select></Field></div>;
  if (type === 'accounts') return <div className="form-grid">{text('name', 'Name', 'text', true)}{text('institution', 'Institution')}<Field label="Type"><select value={values.account_type} onChange={(e) => set('account_type', e.target.value)}>{accountTypeOptions.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></Field>{text('opening_balance', 'Opening balance')}</div>;
  return <div className="form-grid">{text('name', 'Name', 'text', true)}</div>;
}
