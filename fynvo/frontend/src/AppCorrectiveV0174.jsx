...
export default function AppCorrectiveV0174() {
  ...
  useEffect(() => {
    const onKeyDown = (event) => {
      if (event.key === 'Escape') {
        ...
        else if (mobileNavOpen) { event.preventDefault(); setMobileNavOpen(false); window.requestAnimationFrame(() => menuButtonRef.current?.focus({ preventScroll: true })); }
      }
    };
  ...
  async function saveEdit(e) {
    ...
    const message = `${creating ? edit.label.replace(/^New /, '') + ' created.' : edit.label + ' updated.'}`;
    setEdit(null); setSuccess(message); await loadAll();
  }
...
function Overview({ data, setActive, rangeDays, setQuick, quickDefaults, loading }) {
  const command = data.command || {}; const kpis = command.kpis || {}; const forecast = command.forecast?.baseline; const expected = command.forecast?.expected; const goals = command.goals || data.goals || []; const planned = command.top_planned_spending || data.planned || []; const attention = command.attention || {};
...
function RecordTable({ active, data, onEdit }) { const cfg = { Accounts: ['accounts', data.accounts, 'Account'], Transactions: ['transactions', data.transactions, 'Transaction'], Income: ['income', data.income, 'Income'], Bills: ['bills', data.bills, 'Bill'], 'Planned Spending': ['planned', data.planned, 'Planned Spending'] }[active]; const [type, rows, label] = cfg; return <section className="panel"><div className="panel-head"><div><h2>{active}</h2></div><button className="primary ghost" onClick={() => onEdit({ type, label: `New ${label}`, row: { id: null }, values: normaliseRecord(type, {}) })}>+ Add</button></div>{rows.length ? <div className="table">{rows.map((row) => <div className="tr" key={row.id}><span>{dateLabel(row.date || row.transaction_date || row.due_date || row.next_payment_date || row.planned_date)}</span><span>{row.name || row.description || row.merchant || row.account_type}</span><span>{money(row.amount || row.estimated_amount || row.current_balance) || ''}</span><span>{active === 'Accounts' ? accountTypeLabel(row.account_type) : row.status || row.completeness || 'Active'}</span><button onClick={() => onEdit({ type, label, row, values: normaliseRecord(type, row) })}>Edit</button></div>)}</div> : <Empty title={`No ${active.toLowerCase()} yet`}>Use Add to create the first record.</Empty>}</section>; }
...
function DynamicFields({ type, values, set, data }) {
  ...
  if (type === 'categories') return <div className="form-grid">{text('name', 'Name', 'text', true)}<Field label="Parent category"><select value={values.parent_id || ''} onChange={(e) => set('parent_id', e.target.value)}><option value="">None (parent category)</option>{(data.categories || []).filter((row) => row.parent_id == null && Number(row.id) !== Number(edit?.row?.id)).map((row) => <option key={row.id} value={row.id}>{row.name}</option>)}</select></Field><Field label="Type"><select value={values.category_type} onChange={(e) => set('category_type', e.target.value)}><option>expense</option><option>income</option><option>transfer</option></select></Field>{text('notes', 'Notes')}</div>;
  if (type === 'accounts') return <div className="form-grid">{text('name', 'Account name', 'text', true)}{text('opening_balance', 'Opening balance', 'text', true)}<Field label="Account type"><select value={values.account_type} onChange={(e) => set('account_type', e.target.value)}>{accountTypeOptions.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></Field>{text('institution', 'Institution')}</div>;
